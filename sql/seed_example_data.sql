-- ProcureEase example data (realistic labels)
-- Assumes you already have `users` and `department` rows.
-- Recommended flow:
--   1) Run sql/cleanup_keep_users.sql
--   2) Run this script
-- Notes:
--   - `dept_seed` below is a CTE (not a real table). If you run only the UPDATE snippet,
--     you'll get: relation "dept_seed" does not exist. Run the whole WITH...SELECT block.
--   - If you only want to fill details for dept_id=1, run sql/update_department_1_details.sql
